...
import { useDatos, ORIGEN, FECHAS, ORIGENES, DESTINOS_ALL, DC, STATUS_CFG, requerimiento, EMPTY_TRAILER } from "../store/datos";

let nextId = 100;

export default function Modulo3() {
  const { trailers, setTrailers } = useDatos();
  const [diaFil, setDiaFil] = useState("Lun 26");
  const [modal, setModal] = useState(null);
  const [form, setForm] = useState({});

  const hoy = trailers.filter((t) => t.fecha === diaFil);
  const reqsHoy = requerimiento.filter((r) => r.fecha === diaFil);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-base font-semibold text-gray-900">Módulo 3 — Tablero de trailers</h1>
          <p className="text-sm text-gray-500 mt-0.5">Vista Mónica · {diaFil}</p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-2 mb-4">
        {[
          { l: "Solicitados", v: totalSol, c: "text-gray-900" },
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden mb-6">
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 bg-gray-50">
          <span className="text-sm font-semibold text-gray-900">Trailers registrados hoy</span>
        ...
        {hoy.length === 0 ? (
          <div className="text-xs text-gray-400 text-center py-6 italic">Ningún trailer registrado hoy</div>
        ) : (
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-semibold text-gray-800">Por destino</span>
      </div>

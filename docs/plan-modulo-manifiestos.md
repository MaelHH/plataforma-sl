...
- **Schema HANA = resuelto solo (empresa-aware):** `_hana_schema()` en `queries.py` ya usa `get_company_actual()` (el Company SAP de la empresa, de MySQL vía multiempresa) y solo cae al `.env` si no hay empresa. **No hace falta config aparte.** La infra HANA (cliente solo-SELECT `hana_client.py` + patrón `fletes_acarreo`) está lista.
- **ÚNICO pendiente para construir:** los **nombres reales de columnas** (`Code`, `Name`, `U_...`) de `@P_PALLETS` y `@P_PALLET_ASGMNT` — correr `SELECT * FROM "@P_PALLETS" LIMIT 5` en el Generador de Consultas y pegar los encabezados. (Ojo: los nombres visibles "Número de cajas"/"ItemCode" son descripciones; se necesitan los de la columna SQL.)
